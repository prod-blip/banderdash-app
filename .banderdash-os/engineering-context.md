...
- an export bundler package foundation with typed `ExportManifest` metadata, runtime validation, unique custom-element tag generation, immutable artifact writing for JS/manifest/preview files, and backend export records;
...
Export cleanup, Claude/Codex provider adapters, CompareToggle component path, and browser-backed QA execution are not implemented yet.
...
1. Add export cleanup for old completed exports and temp artifacts.
2. Wire export controls into the editor workflow.
3. Add additional provider adapters only if needed for local workflow verification.

## MVP Plan Progress

`implementation-plan.md` currently lists 42 implementation tasks. Tasks 0.1, 0.2, 1.1, 1.2, 1.3, 1.4, 2.1, 2.2, 2.3, 2.4, 2.5, 4.1, 4.2, 4.3, 5.1, 5.2, 5.3, 6.2, 6.3, 6.4, 6.5, 8.1, 8.2, 9.1, 9.2, 9.3, 10.1, 10.2, 11.1, 11.2, 12.1, 12.2, and 12.3 are landed on `main`, leaving 9 plan tasks. Do not treat that as the exact required PR count; the expected remaining reviewable PR count is roughly 9–13 if closely related small tasks are grouped carefully.
