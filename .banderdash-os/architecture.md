...
- `backend/src/nodes/analyst.ts` runs the Analyst node through `LLMProvider.structured(...)` only; it checks structured-output capability, builds article/block-ID grounded prompts, rejects decorative/shallow/jargon-only/vague suggestions in the prompt boundary, validates returned candidate shape and article/version/block/span references, and persists proposed candidates to SQLite.
- `backend/src/nodes/critic.ts` runs the Critic node through `LLMProvider.structured(...)` only; it applies the enact-meaning-not-decoration rule to proposed candidates, explicitly rejects decoration, shallow comparison, glossary-only jargon, vague thematic suggestions, and prose-sufficient cases, requires exactly one decision for each input candidate, prevents ID/reference/pattern drift, and persists `survived` or `rejected_by_critic` status updates to SQLite.
...
- The generated export custom element renders article blocks without duplicating the title heading and includes self-contained audited `ReactiveValue` / `CompareToggle` controls from validated library props, with safe fallback text retained inside each interaction section.
